...
export const setVisibilityFilter = actionCreator<string>(
  ...
  return (dispatch: Dispatch<AnyAction>) => {
    ...
    return fetch("http://localhost:3000/todo", {
      ...
      .then(todos => {
        // console.log(todos)
        dispatch(
          LoadTodosAsyncActions.doneLoadTodos({ params: {}, result: todos })
        );
      })
      .catch(error => {
        console.error(error);
        dispatch(
          LoadTodosAsyncActions.failedLoadTodos({ params: {}, error: {} })
        );
      });
  };
};
